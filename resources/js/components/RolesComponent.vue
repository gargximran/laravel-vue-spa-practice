...
<div>
  ...
  >
      ...
      <v-toolbar flat color="secondary ">
        ...
        <v-dialog v-model="dialog" max-width="500px">
          <template v-slot:activator="{ on }">
            <v-btn color="gray darken-4" dark class="mb-2" v-on="on">New Role</v-btn>
          </template>
          <v-form lazy-validation v-model="valid" ref="roleForm">
            <v-card>
              <v-card-title>
                <span class="subheader">{{ formTitle }}</span>
              </v-card-title>
                ...
                <v-container>
                  <v-row>
                    <v-col cols="12" sm="6" md="4">
                          ...
                          <v-text-field :rules="roleRules" error v-model="editedItem.name" label="Name" prepend-icon="mdi-rename-box"></v-text-field>
    ...
    <template v-slot:item.actions="{ item }">
      <v-icon small class="indigo--text mr-2" @click="editItem(item)">mdi-pencil</v-icon>
      <v-icon small class="red--text" @click="openModal(item)">mdi-delete</v-icon>
      ...
      <v-btn color="primary" @click="initialize">Reset</v-btn>
        ...
        <v-dialog v-model="deleteItemDialog" max-width="500px">
          <v-card>
            ...
            <v-card-actions>
              <v-spacer></v-spacer>
              <v-btn color="secondary" class="text-capitalize" text  @click="deleteItemDialog = false">Cancel</v-btn>
              <v-btn color="error darken-1" class="text-capitalize" text @click="deleteItem">Yes! Delete</v-btn>
            </v-card-actions>
          </v-card>
        </v-dialog>
...
export default {
  ...
  methods: {
    ...
    deleteItem() {
      ...
      axios.delete("/api/roles/delete/"+this.toDelete)
      .then(res => {
    ...
    save() {
      if(this.$refs.roleForm.validate()){
        ...
        if (this.editedIndex > -1) {
          ...
          axios.post('/api/roles/update/'+this.editedItem.id, {name: this.editedItem.name})
          .then(res => {          
            this.roles.splice(editedItemIndex, 1, res.data.role)
            this.loadingloder = false
